...
class NigeriaPassportValidator(BaseValidator):
    """
    Validate Nigeria Passport specific requirements.

    Nigeria Passport specifics:
    - Passport number format: 1 letter + 8 digits (e.g., A12345678)
    - MRZ country code: NGA
    - Validity period: 5 years (32-page) or 10 years (64-page) for adults
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        issues = []
        warnings = []
        details = {"checks_performed": [], "country": "Nigeria", "country_code": self.COUNTRY_CODE}
        ...
        # Check 2: Passport number format (1 letter + 8 digits)
        details["checks_performed"].append("passport_number_format")
        clean_number = re.sub(r"[\s\-]", "", document_number.strip().upper())

        nigeria_passport_format = r"^[A-Z]\d{8}$"

        if not clean_number:
            issues.append("Missing passport number")
        elif not re.match(nigeria_passport_format, clean_number):
            if len(clean_number) == 9 and clean_number[0].isalpha():
                warnings.append(
                    f"Passport number '{document_number}' may have format issues. "
                    "Nigeria passport: 1 letter + 8 digits (e.g., A12345678)"
                )
            else:
                issues.append(
                    f"Invalid Nigeria passport format. Expected: 1 letter + 8 digits (e.g., A12345678), "
        ...
        # Check 4: Validity period (5 years for 32-page, 10 years for 64-page)
        ...
        if issue_date_str and expiry_date:
            ...
            if issue_date and exp:
                ...
                # Nigeria passports are valid for 5 or 10 years
                if not (4.5 <= validity_years <= 5.5) and not (9.5 <= validity_years <= 10.5):
                    warnings.append(
                        f"Passport has {validity_years:.1f} year validity. "
                        f"Nigeria passports are typically 5 years (32-page) or 10 years (64-page)."
                    )
                else:
                    details["validity_valid"] = True
                    if 4.5 <= validity_years <= 5.5:
                        details["passport_type"] = "32-page (5 years)"
                    else:
                        details["passport_type"] = "64-page (10 years)"
        ...
        if issues:
            return self._create_result(
                ...
                message=f"Nigeria Passport validation failed: {'; '.join(issues)}",
            ...
            return self._create_result(
                ...
                message="Nigeria Passport validation passed with warnings",
            ...
            return self._create_result(
                ...
                message="Nigeria Passport validation passed",
                details=details,
                execution_time_ms=execution_time
            )
