...
class ColombiaPassportValidator(BaseValidator):
    """
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        issues = []
        warnings = []
        details = {"checks_performed": [], "country": "Colombia", "country_code": self.COUNTRY_CODE}
        ...
        # Check 4: Validity period (10 years for all ages)
        ...
        if issue_date_str and expiry_date:
            ...
            if issue_date and exp:
                ...
                # Colombia passports are valid for 10 years
                if not (9.5 <= validity_years <= 10.5):
                    warnings.append(
                        f"Passport has {validity_years:.1f} year validity. "
                        f"Colombia passports are typically valid for 10 years."
                    )
                else:
        ...
        if issues:
            return self._create_result(
                ...
                message=f"Colombia Passport validation failed: {'; '.join(issues)}",
            ...
            return self._create_result(
                ...
                message="Colombia Passport validation passed with warnings",
            ...
            return self._create_result(
                ...
                message="Colombia Passport validation passed",
                details=details,
                execution_time_ms=execution_time
            )
