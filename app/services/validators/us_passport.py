...
class USPassportValidator(BaseValidator):
    """
    Validate US Passport specific requirements.

    US Passport specifics:
    - Passport number format: 9 digits
    - MRZ country code: USA
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        issues = []
        warnings = []
        details = {"checks_performed": [], "country": "United States", "country_code": self.COUNTRY_CODE}
        ...
        us_passport_format = r"^\d{9}$"

        if not clean_number:
            issues.append("Missing passport number")
        elif not re.match(us_passport_format, clean_number):
            if clean_number.isdigit() and 8 <= len(clean_number) <= 10:
                warnings.append(
                    f"Passport number '{document_number}' has {len(clean_number)} digits. "
                    "US passport numbers are typically 9 digits."
                )
            else:
                issues.append(
                    f"Invalid US passport format. Expected: 9 digits, Got: {document_number}"
                )
        else:
        ...
        if issues:
            return self._create_result(
                ...
                message=f"US Passport validation failed: {'; '.join(issues)}",
            ...
            return self._create_result(
                ...
                message="US Passport validation passed with warnings",
            ...
            return self._create_result(
                ...
                message="US Passport validation passed",
                details=details,
                execution_time_ms=execution_time
            )
