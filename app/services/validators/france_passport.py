...
class FrancePassportValidator(BaseValidator):
    """
    Validate France Passport specific requirements.

    France Passport specifics:
    - Passport number format: 9 alphanumeric characters (2 digits + 2 letters + 5 digits)
    - Example: 12AB34567
    - MRZ country code: FRA
    - Validity period: 10 years for adults (18+), 5 years for minors
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        issues = []
        warnings = []
        details = {"checks_performed": [], "country": "France", "country_code": self.COUNTRY_CODE}
        ...
        # Check 2: Passport number format (9 alphanumeric: 2 digits + 2 letters + 5 digits)
        details["checks_performed"].append("passport_number_format")
        clean_number = re.sub(r"[\s\-]", "", document_number.strip().upper())

        # French passport format: 2 digits + 2 letters + 5 digits = 9 characters
        france_passport_format = r"^\d{2}[A-Z]{2}\d{5}$"
        # Also accept general 9 alphanumeric as fallback
        general_format = r"^[A-Z0-9]{9}$"

        if not clean_number:
            issues.append("Missing passport number")
        elif re.match(france_passport_format, clean_number):
            details["passport_number_valid"] = True
            details["format_type"] = "standard"
        elif re.match(general_format, clean_number):
            details["passport_number_valid"] = True
            details["format_type"] = "alphanumeric"
            warnings.append(
                f"Passport number '{document_number}' accepted but may not match standard "
                "French format (2 digits + 2 letters + 5 digits)."
            )
        else:
            issues.append(
                f"Invalid France passport format. Expected: 9 characters (e.g., 12AB34567), "
        ...
        if issues:
            return self._create_result(
                ...
                message=f"France Passport validation failed: {'; '.join(issues)}",
            ...
            return self._create_result(
                ...
                message="France Passport validation passed with warnings",
            ...
            return self._create_result(
                ...
                message="France Passport validation passed",
                details=details,
                execution_time_ms=execution_time
            )
