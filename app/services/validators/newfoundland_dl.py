...
class NewfoundlandDriversLicenseValidator(BaseValidator):
    """
    Validate Newfoundland and Labrador Driver's Licence specific requirements.

    Newfoundland DL specifics:
    - Licence number format: Letter + 9 digits (e.g., A123456789)
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        # Check 1: Licence number format (Letter + 9 digits)
        ...
        # Newfoundland format: Letter + 9 digits = 10 characters
        nl_format = r"^[A-Z]\d{9}$"

        if not clean_number:
            issues.append("Missing licence number")
        elif re.match(nl_format, clean_number):
            details["licence_number_valid"] = True
            details["licence_letter"] = clean_number[0]
            details["licence_digits"] = clean_number[1:]
        elif len(clean_number) == 10 and clean_number[0].isalpha():
            warnings.append(
                f"Licence number '{document_number}' may have format issues. "
                "Newfoundland licence: Letter + 9 digits (e.g., A123456789)"
            )
            details["licence_letter"] = clean_number[0]
        else:
            issues.append(
                f"Invalid Newfoundland licence format. Expected: Letter + 9 digits (e.g., A123456789), "
        ...
        # Check 3: Minimum age for Newfoundland DL (16 for Class 7 Learner)
        details["checks_performed"].append("minimum_age_newfoundland")
        if date_of_birth:
            ...
            if dob:
                ...
                if age < 16:
                    issues.append(
                        f"Person is {age} years old. Newfoundland requires minimum 16 for Class 7 Learner"
                    )
                elif age < 18:
                    warnings.append(
                        f"Person is {age}. May hold Class 7 (Learner) licence"
                    )

        # Check 4: Expiry date validation (Newfoundland DL expires on birthday)
            ...
            if dob and exp:
                if dob.month == exp.month and dob.day == exp.day:
                    ...
                    warnings.append(
                        ...
                        f"({dob.strftime('%m-%d')}). Newfoundland DL typically expires on holder's birthday"
                    )

        # Check 5: Licence validity period (typically 4 years in Newfoundland)
            ...
            if issue_date and exp:
                ...
                if validity_years < 1.5:
                    ...
                    warnings.append(
                        ...
                        "4-year Newfoundland licence term"
                    )
                    ...
                    if dob and issue_date < dob:
                        ...
                        if age_at_issue < 16:
                            issues.append(
                                ...
                                "Newfoundland requires minimum 16 for any licence"
                            )
        ...
        if issues:
            return self._create_result(
                ...
                message=f"Newfoundland Driver's Licence validation failed: {'; '.join(issues)}",
                details={
                    **details,
                    "issues": issues,
                    "warnings": warnings
            ...
            return self._create_result(
                ...
                message="Newfoundland Driver's Licence validation passed with warnings",
                details={
                    **details,
                    "warnings": warnings
                },
            ...
            return self._create_result(
                ...
                message="Newfoundland Driver's Licence validation passed",
                details=details,
                execution_time_ms=execution_time
            )
