...
class NunavutDriversLicenseValidator(BaseValidator):
    """
    Validate Nunavut Driver's Licence specific requirements.

    Nunavut DL specifics:
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        # Nunavut format: 6 digits
        nu_format = r"^\d{6}$"

        if not clean_number:
            issues.append("Missing licence number")
        elif re.match(nu_format, clean_number):
            ...
            warnings.append(
                ...
                "Nunavut licence typically has 6 digits."
            )
            details["licence_digits"] = len(clean_number)
        else:
            issues.append(
                f"Invalid Nunavut licence format. Expected: 6 digits (e.g., 123456), "
                f"Got: {document_number}"
            )

        details["clean_licence_number"] = clean_number

        # Check 2: Minimum age for Nunavut DL (15 for Class 7 Learner)
        details["checks_performed"].append("minimum_age_nunavut")
        if date_of_birth:
            ...
            if dob:
                ...
                if age < 15:
                    issues.append(
                        f"Person is {age} years old. Nunavut requires minimum 15 for Class 7 Learner"
                    )
                elif age < 16:
                    warnings.append(
        ...
        # Check 3: Expiry date validation (Nunavut DL expires on birthday)
            ...
            if dob and exp:
                if dob.month == exp.month and dob.day == exp.day:
                    ...
                    warnings.append(
                        ...
                        f"({dob.strftime('%m-%d')}). Nunavut DL typically expires on holder's birthday"
                    )
                    ...
                    if dob and issue_date < dob:
                        ...
                        if age_at_issue < 15:
                            issues.append(
                                ...
                                "Nunavut requires minimum 15 for any licence"
                            )
        ...
        if issues:
            return self._create_result(
                ...
                message=f"Nunavut Driver's Licence validation failed: {'; '.join(issues)}",
                details={
                    **details,
                    "issues": issues,
                    "warnings": warnings
            ...
            return self._create_result(
                ...
                message="Nunavut Driver's Licence validation passed with warnings",
                details={
                    **details,
                    "warnings": warnings
                },
            ...
            return self._create_result(
                ...
                message="Nunavut Driver's Licence validation passed",
                details=details,
                execution_time_ms=execution_time
            )
