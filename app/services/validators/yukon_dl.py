...
class YukonDriversLicenseValidator(BaseValidator):
    """
    Validate Yukon Driver's Licence specific requirements.

    Yukon DL specifics:
    - Licence number format: 6 digits (e.g., 123456)
    - Name format on licence: "LASTNAME FIRSTNAME" or "LASTNAME, FIRSTNAME"
    - Licence classes: Class 7 (Learner), Class 5 (Full)
    - Minimum age: 15 for Class 7 Learner (with parent/guardian consent)
    - Expiry is typically on the holder's birthday
    - Validity period: 5 years for most licences
    - Small population territory in Canada's north
    """

    name = "yukon_drivers_license"

    def _extract_last_name(self, document_data: Dict[str, Any]) -> Tuple[str, str]:
        """
        Extract last name from document data.

        Yukon DL uses "LASTNAME FIRSTNAME" or "LASTNAME, FIRSTNAME" format.
    ...
    async def validate(self, document_data: Dict[str, Any]) -> ValidatorResult:
        ...
        # Check 1: Licence number format (6 digits)
        ...
        # Yukon format: 6 digits
        yk_format = r"^\d{6}$"

        if not clean_number:
            issues.append("Missing licence number")
        elif re.match(yk_format, clean_number):
            details["licence_number_valid"] = True
            details["licence_digits"] = 6
        elif clean_number.isdigit() and 5 <= len(clean_number) <= 7:
            warnings.append(
                f"Licence number '{document_number}' has {len(clean_number)} digits. "
                "Yukon licence typically has 6 digits."
            )
            details["licence_digits"] = len(clean_number)
        else:
            issues.append(
                f"Invalid Yukon licence format. Expected: 6 digits (e.g., 123456), "
                f"Got: {document_number}"
            )

        details["clean_licence_number"] = clean_number

        # Check 2: Minimum age for Yukon DL (15 for Class 7 Learner with consent)
        details["checks_performed"].append("minimum_age_yukon")
        if date_of_birth:
            ...
            if dob:
                ...
                if age < 15:
                    issues.append(
                        f"Person is {age} years old. Yukon requires minimum 15 for Class 7 Learner"
                    )
                elif age < 16:
                    warnings.append(
                        f"Person is {age}. Holds Class 7 (Learner) licence (requires parental consent)"
                    )
                elif age < 18:
                    warnings.append(
                        f"Person is {age}. May hold Class 7 or novice Class 5 licence"
                    )

        # Check 3: Expiry date validation (Yukon DL expires on birthday)
            ...
            if dob and exp:
                if dob.month == exp.month and dob.day == exp.day:
                    ...
                    warnings.append(
                        ...
                        f"({dob.strftime('%m-%d')}). Yukon DL typically expires on holder's birthday"
                    )
                    ...
                    if dob and issue_date < dob:
                        ...
                        if age_at_issue < 15:
                            issues.append(
                                ...
                                "Yukon requires minimum 15 for any licence"
                            )
        ...
        if issues:
            return self._create_result(
                ...
                message=f"Yukon Driver's Licence validation failed: {'; '.join(issues)}",
                details={
                    **details,
                    "issues": issues,
                    "warnings": warnings
            ...
            return self._create_result(
                ...
                message="Yukon Driver's Licence validation passed with warnings",
                details={
                    **details,
                    "warnings": warnings
                },
            ...
            return self._create_result(
                ...
                message="Yukon Driver's Licence validation passed",
                details=details,
                execution_time_ms=execution_time
            )
